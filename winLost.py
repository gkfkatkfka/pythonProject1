    ...
    for month in list_month:
        ...
        for idx, tr in enumerate(trs):
            if idx > 0:
                tds = tr.find_all('td')
                if len(tds)==8:
                    tds.insert(0,searchList[-1][1])
                    temp = [year,tds[0], tds[1].text.strip(), tds[2].text.strip()]
                else:
                    temp = [year,tds[0].text.strip(), tds[1].text.strip(), tds[2].text.strip()]
                if tds[8].text.strip()=='-':
                    searchList.append(temp)

# csv 만들기
data = pd.DataFrame(searchList)
data.columns = ['year','day', 'time', 'play']
data.head()
data.to_csv('승패기록.csv', encoding='cp949')
